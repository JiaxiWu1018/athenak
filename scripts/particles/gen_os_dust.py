...
The companion pgen ``src/pgen/nr_pic_os.cpp`` sets the matching conformally-flat,
time-symmetric metric from the same (M, R0) -- keep ``<problem> os_mass`` and
``os_radius_over_mass`` IN SYNC with --mass / --radius here (the t=0 Hamiltonian-
constraint test fails loudly on a mismatch).
