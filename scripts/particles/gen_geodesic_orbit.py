#!/usr/bin/env python3
"""Generate single-particle geodesic initial conditions in a Cartesian Kerr-Schild\
 background.

Produces the NRPIC HDF5 IC contract (x,y,z and the COVARIANT spatial 4-velocity\
 u_i = ux,uy,uz)
for the Stage-2 gr_boris geodesic tests. Two modes:

  rest      : particle momentarily at rest (u_i = 0) at areal radius r0 on the x-axis.\
 In a
              stationary spacetime -u_t is conserved, so IPEN should hold at alpha(r0);\
 the
              particle then falls radially inward.
  circular  : equatorial circular geodesic at areal radius r0. Constant-r equatorial\
 motion is
 ...
 the coordinate
              4-velocity direction (1,0,x0*Omega,0), normalize with the local\
 KS 4-metric, and
              lower indices to get u_i. r should stay constant; -u_t (energy) and u_phi\
 (angular
              momentum) are conserved.

The Cartesian Kerr-Schild metric uses g = eta + 2H l l, matching AthenaK's cartesian_ks\
 for the
a=0 limit (verified: alpha = 1/sqrt(1+2M/r)). M=1 by convention here (AthenaK fixes M=1).
...
def ks_metric(x, y, z, M, a):
    """Cartesian Kerr-Schild 4-metric g_{mu nu} (t,x,y,z) and the KS radius r at\
 (x,y,z)."""
    rho2 = x*x + y*y + z*z
...
def main():
    ...
    ap.add_argument("--r0", type=float, default=10.0,
                    help="areal radius of the orbit/start")
    ap.add_argument("--sign", type=float, default=1.0,
                    help="+1 prograde, -1 retrograde (circular)")
    ...
    g, r_chk = ks_metric(pos[0], pos[1], pos[2], M, a)
    ...
    if args.mode == "rest":
        ...
        L = 0.0
        print(f"[rest] r0={r0}  x0={x0:.6f}  alpha(r0)={alpha:.6f}"
        ...
        print(f"           conserved   E=-u_t={E:.6f}   L=u_phi={L:.6f}"
              f"   gamma_t=u^t={ut:.6f}")

    write_particle_table(args.out, [pos[0]], [pos[1]], [pos[2]],
                         [u_dn[0]], [u_dn[1]], [u_dn[2]],
