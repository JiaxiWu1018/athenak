#!/usr/bin/env python3
"""Generate single-particle geodesic initial conditions in a Cartesian Kerr-Schild background.

Produces the NRPIC HDF5 IC contract (x,y,z and the COVARIANT spatial 4-velocity u_i = ux,uy,uz)
...
The Cartesian Kerr-Schild metric uses g = eta + 2H l l, matching AthenaK's cartesian_ks for the
...
def main():
  ...
  ap.add_argument("--r0", type=float, default=10.0, help="areal radius of the orbit/start")
  ap.add_argument("--sign", type=float, default=1.0, help="+1 prograde, -1 retrograde (circular)")
  ...
  if args.mode == "rest":
    ...
    L = 0.0
    print(f"[rest] r0={r0}  x0={x0:.6f}  alpha(r0)={alpha:.6f}  -> IPEN should hold at {alpha:.6f}")
  else:
    Omega = args.sign*np.sqrt(M)/(r0**1.5 + args.sign*a*np.sqrt(M))
    udir = np.array([1.0, 0.0, x0*Omega, 0.0])             # (u^t,u^x,u^y,u^z) direction
    norm = -(udir @ g @ udir)
    if norm <= 0:
      raise SystemExit(f"no timelike circular orbit at r0={r0}, a={a} (norm={norm:.3e}); "
    ...
    print(f"           conserved   E=-u_t={E:.6f}   L=u_phi={L:.6f}   gamma_t=u^t={ut:.6f}")
