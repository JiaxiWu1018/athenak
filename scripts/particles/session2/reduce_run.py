...
def main():
    ...
    # Session 2 needs the l = 1 dipole VECTOR of each equal-rest-mass band, not just its
    # magnitude, because the sign of cos(D_q, D_q') is what separates a coherent
    # whole-cluster translation (neighbouring shells aligned) from an internal sloshing
    # mode (neighbouring shells anti-aligned), and it is also what shows whether the
    # dominant direction wanders or locks.  |D| equals A_1 exactly for this definition,
    # so the magnitude column is a consistency check against modes.csv.
    fd = open(os.path.join(a.out, "dipoles.csv"), "w")
    fd.write("time,t_over_P,band,Dx,Dy,Dz,absD,n_used,A_shot\n")
    ...
    for path in files:
        ...
        # per-band dipole vectors, same CoM reference as the `m` bands in modes.csv
        fd.write("%.10g,%.10g,com,%.10e,%.10e,%.10e,%.10e,%d,%.10e\n"
                 % (t, tp, d1[0], d1[1], d1[2], float(np.linalg.norm(d1)),
                    sel.size, shot))
        for gi in range(a.macro):
            gl = groups_local[gi]
            if gl.size == 0:
                continue
            dg = dipole_vector(nhc[gl, 0], nhc[gl, 1], nhc[gl, 2])
            fd.write("%.10g,%.10g,m%d,%.10e,%.10e,%.10e,%.10e,%d,%.10e\n"
                     % (t, tp, gi, dg[0], dg[1], dg[2], float(np.linalg.norm(dg)),
                        gl.size, (gl.size/2.0)**-0.5))
        rmed = np.median(rrel)
        inner = rrel < rmed
