...
ABORT_TIMEOUT = 600.0   # a run that is supposed to abort must not be able to hang CI
...
def run_args(args, threads=1, expect_failure=False):
    ...
    ``expect_failure`` inverts the exit-status assertion, for regressions whose whole
    point is that the code must refuse to continue (a fatal diagnostic). The log is
    returned either way so the caller can assert on the message itself -- a nonzero exit
    alone would also be satisfied by an unrelated crash.
    """
