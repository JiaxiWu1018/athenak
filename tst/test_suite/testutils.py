...
def run_command(command: List[str], text: bool = False, timeout: float = None) -> bool:
    ...
    Args:
        ...
        timeout (float): Optional wall-clock limit in seconds. Without one, a test that
            drives a failure path can hang the whole suite forever if the failure ever
            becomes rank-partial: the surviving ranks block in the next MPI collective
            and communicate() never returns. Default None preserves the previous
