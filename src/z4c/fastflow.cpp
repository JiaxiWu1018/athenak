...
//! \brief publish the just-converged surface for consumers, unless part of it is
//! off-grid. Called from Find() on a converged flow, where a0/ac/as and `rr` are
//! mutually consistent; the surface is rank-replicated, so this is local.
//!
//! ON-GRID GATE: SurfaceIntegrals accumulates only where havepoint != 0, so a surface
//! reaching past the mesh can "converge" on a partial integral; excising against one
//! removes matter nowhere near a horizon, so the snapshot refuses it (FastFlow's own
//! ah_found and summary output are unchanged). havepoint is per-rank -- reduce with
//! MAX, since SUM would miscount a point claimed by two ranks through ghost zones.
