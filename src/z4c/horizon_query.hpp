...
//!
//! FastFlow represents a surface as a spectral graph R(theta,phi) over the unit sphere
//! about FastFlow::center; this header evaluates that same sum through the finder's own
//! SphericalHarm() (the surface must not be re-derived from a second harmonic
//! implementation). A consumer stages surfaces into two flat host-filled views,
//!
//!   par (nhorizon, NAH_PAR)  {cx, cy, cz, rmin, rmax, valid}
//!   coef(nhorizon, ncoef)    [ a0(0..lmax) | ac(0..lmpoints-1) | as(0..lmpoints-1) ]
//!
//! and calls AHInside(). rmin/rmax are the surface's angular extrema, so most points
//! resolve by bracket without paying the O(lmax^2) harmonic sum.
