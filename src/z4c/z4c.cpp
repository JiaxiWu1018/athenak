...
Z4c::Z4c(MeshBlockPack *ppack, ParameterInput *pin) :
  ...
  if (shift_driver == "legacy") {
    ...
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl << "z4c/shift_driver='" << shift_driver
  ...
  if (!opt.const_damp) {
    opt.m_min = std::numeric_limits<Real>::max();
    for (int i = 0; i < static_cast<int>(ptracker.size()); ++i) {
      if (!ptracker[i]->IsBlackHole()) {
        continue;
      }
      const Real mass = ptracker[i]->GetMass();
      if (!(mass > 0.0) || !std::isfinite(mass)) {
        std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
                  << std::endl << "z4c/const_damp=false requires every black-hole "
                  << "tracker mass to be finite and positive; co_" << i
                  << "_mass=" << mass << "." << std::endl;
        std::exit(EXIT_FAILURE);
      }
      damping_tracker_indices.push_back(i);
      opt.m_min = std::min(opt.m_min, mass);
    }
    if (damping_tracker_indices.empty()) {
      std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
                << std::endl << "z4c/const_damp=false requires at least one "
                << "black-hole compact-object tracker." << std::endl;
      std::exit(EXIT_FAILURE);
    }
    damping_puncture_info = DualArray2D<Real>("damping puncture info",
                                              damping_tracker_indices.size(), NDIM + 1);
  }
