...
// ParticlesBoundaryValues::SetNewPrtclGID. The value is what MarkExcised writes into
// Particles::excise_flag, what the `reason` column of the death-record CSV encodes, and
// what indexes every per-reason counter in the ledger chain (Particles::ndestroy_thisrank
// -> ParticlesBoundaryValues::ndest_global -> Mesh::nprtcl_destroyed_cum). Widening this
// means widening all of those plus the device counter in SetNewPrtclGID; the restart
// file's fixed 4-Real particle header is NOT widened (see outputs/restart.cpp).
