...
void Particles::GR_BorisPush() {
  ...
  KOKKOS_LAMBDA(const int p) {
    ...
    if (!find_root) {
      Kokkos::atomic_add(&nfail(0), static_cast<uint64_t>(1));
    }
  ...
  // Printing one line per failed particle can generate multi-gigabyte logs precisely
  // when a model becomes poorly resolved.  Keep an exact device count and report one
  // global cumulative summary at a configurable cycle interval instead.
  int ncycle = pmy_pack->pmesh->ncycle;
  if (gr_boris_warning_interval > 0 &&
      ncycle % gr_boris_warning_interval == 0) {
    auto hfail = Kokkos::create_mirror_view(gr_boris_failures);
    Kokkos::deep_copy(hfail, gr_boris_failures);
    uint64_t global_failures = hfail(0);
#if MPI_PARALLEL_ENABLED
    MPI_Allreduce(MPI_IN_PLACE, &global_failures, 1, MPI_UINT64_T, MPI_SUM,
                  pbval_part->mpi_comm_part);
#endif
    if (global_variable::my_rank == 0 &&
        global_failures > gr_boris_failures_reported) {
      std::cout << "WARNING: gr_boris cumulative forward-Euler fallbacks at cycle "
                << ncycle << ": " << global_failures << " ("
                << global_failures - gr_boris_failures_reported
                << " since the previous report)" << std::endl;
    }
    gr_boris_failures_reported = global_failures;
  }
}
