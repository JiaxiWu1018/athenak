...
//! Checked in that order, so the recorded reason follows the precedence
//! exit > sphere > lapse > horizon (exit is applied later, in SetNewPrtclGID); every
//! enabled criterion still destroys. Marking uses the pre-wrap position and evaluates
//! the post-step (n+1) fields, consistent with the post-push position.
//!
//! WHICH horizon surface: Z4c::FindHorizon is a final-stage stagen task run after
//! ConvertZ4cToADM publishes t^{n+1}, and Push/MarkExcised run afterwards, so the
//! criterion tests the horizon found THIS cycle from the geometry the push used. If the
//! find did not converge or was refused, StageHorizons falls back to the last published
//! surface -- never to FastFlow::ah_found (see fastflow.hpp).
