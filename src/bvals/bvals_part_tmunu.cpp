...
//! then an Allgatherv of {sendrank,recvrank,count} tuples on mpi_comm_part with a
//! temporary contiguous three-int datatype and distinct message tags (2 = Reals,
//! 3 = ints). Its census is separate from migration, which runs in an earlier task.
...
void ParticlesBoundaryValues::ExchangeTmunuImages() {
  ...
  // using the current migration path's temporary contiguous three-int datatype. This is
  // separate from the migration census, which completed in an earlier task this cycle.
