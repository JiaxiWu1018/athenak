...
TaskStatus ParticlesBoundaryValues::SetNewPrtclGID() {
  ...
  // destruction marking (Stage 3c): device counters {0: append slot, 1..N: per-reason
  // counts, indexed by ParticlesDeathReason} and the destroyed-side {sum tag, sum tag^2}
  // checksum accumulators of the two-sided conservation ledger (written only when
  // debug >= 1). The kernel does atomic_add(&dstc(1+reason)), so this MUST be
  // 1 + NPRTCL_DEATH_REASON wide.
  ...
  par_for("part_update",DevExeSpace(),0,(npart-1), KOKKOS_LAMBDA(const int p) {
    ...
    // non-periodic boundary destroys the particle (PrtclDeathExit), and the excision
    // criteria destroy via the MarkExcised flags (sphere / lapse / horizon; excised
    // particles need not have crossed anything). Exit takes precedence.
    ...
    if (crossed) {
      ...
      if (indx >= 0) {
        ...
        // No destination: not a recoverable state, so the host aborts below. The GID is
        // left alone meanwhile only to avoid shipping a dangling one to MPI. Mesh exits
        // are destroyed before the search, so exactly two causes reach here:
        //
        //   (a) overspeed -- |offset| == 2 somewhere, i.e. the new position is more than
        //       one MeshBlock width outside the block the particle started in, so it
        //       moved further in one update than the 26-neighbour search can represent.
        //       Fatal by design rather than repaired by a whole-mesh lookup: it means an
        //       assumption broke (interpolated geometry, gauge excursion, or a timestep
        //       the particle CFL does not bound), and re-homing would hide that.
        //   (b) broken neighbour contract -- offsets all within +-1, so the motion IS
        //       supported, but no neighbour slot covers that direction. The mesh, not the
        //       particle, is wrong (2:1 balance / SetNeighbors). Equally fatal.
        //
        // Both report unconditionally: under debug = 0 the old behaviour was a silent
        // stale GID.
