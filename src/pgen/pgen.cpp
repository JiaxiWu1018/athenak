...
ProblemGenerator::ProblemGenerator(ParameterInput *pin, Mesh *pm, IOWrapper resfile,
  ...
  if (pm->pmb_pack->ppart != nullptr) {
    ...
    // destroyed ledger so conservation accounting spans restart segments. Still 4 Reals
    // by design (see outputs/restart.cpp); the horizon count rides in the parameter dump
    // and defaults to 0, so files written before it existed restore a correct ledger.
