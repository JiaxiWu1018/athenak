...
//!
//! mode=farhop: migration-range fixture. For every local MeshBlock, every one of the 26
//! offset directions and every requested hop count K (<problem> hops, default "2,3"),
//! place one particle at the block CENTRE with the velocity that carries it exactly K
//! block widths per particle step (dt = cfl * smallest cell in the mesh). The tag
//!     tag = (gid*27 + (ix+1) + 3*(iy+1) + 9*(iz+1))*8 + K
//! is globally unique and decodable. K = 1 is an ordinary single-neighbour crossing and
//! must migrate cleanly; K >= 2 exceeds the supported migration range and must abort (see
//! bvals_part.cpp). Requires a strictly periodic mesh so no hop is instead a mesh exit.
//!
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (mode.compare("targeted") == 0) {
    ...
    // hop counts, e.g. "2,3": whole MeshBlock widths travelled per particle step.
    // K = 1 is supported; K >= 2 must be fatal.
    std::vector<int> hops;
    {
