...
struct torus_pgen {
  ...
  Real ang_mom;                               // specific angular momentum of the disk
  Real ut_in;                                 // total energy density at the inner edge
};
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  torus.r_outer_edge = pin->GetReal("problem", "r_outer_edge");
  ...
  torus.ang_mom = pin->GetOrAddReal("problem", "angular_momentum", 5.0);
  torus.ut_in = pin->GetOrAddReal("problem", "energy_inner", -0.991);
  torus.kappa = pin->GetOrAddReal("mhd", "kappa", 1.e-2);
  ...
  // don't need to find "outer edge" of torus, set in parfile.
  ...
  if (pmbp->padm != nullptr) {
    ...
    Real a = M * torus.spin; // Spin per unit mass
    ...
    KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
      ...
      if (rUIUC * Sth < 1.0e-10) {
        g0[0][0] = (a * a + pow(rp + 4 * fabs(xS[2]), 4) / (256 * xS[2] * xS[2])) /
                   (xS[2] * xS[2]);
        g0[0][1] = 0;
        g0[0][2] = 0;
        g0[1][1] = g0[0][0];
        g0[1][2] = 0;
        g0[2][2] = -pow(rp + 4 * fabs(xS[2]), 2) * g0[0][0] / 
                   (a * a - 2 * (M * rp + 8 * xS[2] * xS[2]) + 8 * fabs(xS[2]) *
                    (M - 3 * sqrt(M * M - a * a)));
        g0[1][0] = g0[0][1];
        g0[2][0] = g0[0][2];
        g0[2][1] = g0[1][2];
        ...
        g0[0][0] = (gthth * pow(Cph * Cth, 2)) / pow(rUIUC, 2) + (gphph * pow(Sph, 2)) /
                   (pow(rUIUC, 2) * pow(Sth, 2)) + grr * pow(Cph, 2) * pow(Sth, 2);
        g0[0][1] = (gthth * Cph * pow(Cth, 2) * Sph) / pow(rUIUC, 2) - (gphph * Cph * Sph) /
                   (pow(rUIUC, 2) * pow(Sth, 2)) + grr * Cph * Sph * pow(Sth, 2);
        g0[0][2] = grr * Cph * Cth * Sth - (gthth * Cph * Cth * Sth) / pow(rUIUC, 2);
        g0[1][1] = (gphph * pow(Cph, 2)) / (pow(rUIUC, 2) * pow(Sth, 2)) +
                   (gthth * pow(Cth, 2) * pow(Sph, 2)) / pow(rUIUC, 2) +
                   grr * pow(Sph, 2) * pow(Sth, 2);
        ...
        K0[0][0] = (-2 * Krph * Cph * Sph) / rUIUC -
                   (2 * Kthph * Cph / Tth * Sph) / pow(rUIUC, 2);
        K0[0][1] = (Krph * pow(Cph, 2)) / rUIUC + (Kthph * pow(Cph, 2) / Tth) / pow(rUIUC, 2) -
                   (Krph * pow(Sph, 2)) / rUIUC - (Kthph / Tth * pow(Sph, 2)) / pow(rUIUC, 2);
      ...
      Real detg = -g_dd[0][2] * g_dd[0][2] * g_dd[1][1] + 2 * g_dd[0][1] * g_dd[0][2] * g_dd[1][2] -
                  g_dd[0][0] * g_dd[1][2] * g_dd[1][2] - g_dd[0][1] * g_dd[0][1] * g_dd[2][2] +
                  g_dd[0][0] * g_dd[1][1] * g_dd[2][2];
  ...
  KOKKOS_LAMBDA(const int &idx, Real &max_ptot) {
    ...
    // The construction follows Abramowicz et al. 1978
    Real g_dd[3][3] = {adm.g_dd(m, 0, 0, k, j, i), adm.g_dd(m, 0, 1, k, j, i), adm.g_dd(m, 0, 2, k, j, i),
                       adm.g_dd(m, 0, 1, k, j, i), adm.g_dd(m, 1, 1, k, j, i), adm.g_dd(m, 1, 2, k, j, i),
                       adm.g_dd(m, 0, 2, k, j, i), adm.g_dd(m, 1, 2, k, j, i), adm.g_dd(m, 2, 2, k, j, i)};
    Real beta_u[3] = {adm.beta_u(m, 0, k, j, i), adm.beta_u(m, 1, k, j, i), adm.beta_u(m, 2, k, j, i)};
    Real betalow[3] = {0.0};
    for (int ii = 0; ii < 3; ++ii) {
      for (int jj = 0; jj < 3; ++jj) {
        betalow[ii] += g_dd[ii][jj] * beta_u[jj];
      }
    }
    Real beta2 = betalow[0] * beta_u[0] + betalow[1] * beta_u[1] + betalow[2] * beta_u[2];
    Real r = sqrt(x1v * x1v + x2v * x2v + x3v * x3v);
    Real beta_r = (x1v * betalow[0] + x2v * betalow[1]) / (r + 1.e-20);
    beta2 -= beta_r * beta_r;
    Real beta_phi = (x1v * beta_u[1] - x2v * beta_u[0]) / (r * r);
    Real gtt = -1. * adm.alpha(m , k, j, i) * adm.alpha(m , k, j, i) + beta2;
    Real gtp = x1v * betalow[1] - x2v * betalow[0];
    Real gpp = x1v * x1v * g_dd[1][1] + x2v * x2v * g_dd[0][0] - 2 * x1v * x2v * g_dd[0][1];
    Real varpi2 = gtp * gtp - gtt * gpp;
    Real d2 = gtt * trs.ang_mom * trs.ang_mom + 2 * gtp * trs.ang_mom + gpp;
    Real ut = -1.0;
    if ((d2 > 0.0) && (varpi2 > 0.0)) {
      ut = -1. * sqrt(varpi2 / d2);
    }
    Real Omega = -1. * (gtp + gtt * trs.ang_mom) / (gpp + gtp * trs.ang_mom);
    Real utu = sqrt(fabs(1.0 / (gtt + 2 * Omega * gtp + gpp * Omega * Omega)));
    Real h = trs.ut_in / ut;

    // Set atmospheric value
    w0_(m,IDN,k,j,i) = trs.dexcise;
    if (!use_dyngr) {
      w0_(m,IEN,k,j,i) = trs.pexcise / gm1;
    } else {
      w0_(m,IPR,k,j,i) = trs.pexcise;
    }
    w0_(m,IVX,k,j,i) = 0.0;
    w0_(m,IVY,k,j,i) = 0.0;
    w0_(m,IVZ,k,j,i) = 0.0;
    ...
    // If in disk, set the disk value
    if (h > 1.0 && d2 > 0.0 && r < trs.r_outer_edge && r > trs.r_edge) {
      w0_(m,IDN,k,j,i) = pow(gm1 / (trs.gamma_adi) * (h - 1.) / trs.kappa, 1. / gm1);
      if (!use_dyngr) {
        w0_(m,IEN,k,j,i) = 0;
      } else {
        w0_(m,IPR,k,j,i) = trs.kappa * pow(w0_(m,IDN,k,j,i), trs.gamma_adi);
      }
      Real Wvp = utu * Omega;
      w0_(m,IVX,k,j,i) = -1. * x2v * (Wvp + utu * beta_phi);
      w0_(m,IVY,k,j,i) = x1v * (Wvp + utu * beta_phi);
      // Calculate radiation variables (if radiation enabled, not consistent)
      if (is_radiation_enabled) {
        Real log_h = LogHAux(trs, r, 0) - trs.log_h_edge;
        Real ptot_over_rho = gm1/trs.gamma_adi * (exp(log_h) - 1.0);
        Real temp = CalculateT(trs, w0_(m,IDN,k,j,i), ptot_over_rho);
        urad = trs.arad * SQR(SQR(temp));
      }
    }

    // Account for tilt (not written)

    // Set coordinate frame intensity (if radiation enabled) (not consistent)
    if (is_radiation_enabled) {
      Real uu1 = w0_(m,IVX,k,j,i), uu2 = w0_(m,IVY,k,j,i), uu3 = w0_(m,IVZ,k,j,i);
      Real q = g_dd[0][0]*uu1*uu1 + 2.0*g_dd[0][1]*uu1*uu2 + 2.0*g_dd[0][2]*uu1*uu3
             + g_dd[1][1]*uu2*uu2 + 2.0*g_dd[1][2]*uu2*uu3
             + g_dd[2][2]*uu3*uu3;
      Real uu0 = sqrt(1.0 + q);
      Real u_tet_[4];
