...
def output_paths(fname):
    root, _ = os.path.splitext(fname)
    athdf_name = root + ".athdf"
    return athdf_name, athdf_name + ".xdmf"
...
def main(**kwargs):
    ...
    if kwargs['skip_existing']:
        pending_files = []
        for fname in files:
            athdf_name, xdmf_name = output_paths(fname)
            if os.path.exists(athdf_name) and os.path.exists(xdmf_name):
                if kwargs['verbose']:
    ...
    for count, fname in enumerate(files, start=1):
        if kwargs['verbose']:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skip-existing', action='store_true',
                        help='skip bins whose .athdf and .xdmf outputs both exist')
